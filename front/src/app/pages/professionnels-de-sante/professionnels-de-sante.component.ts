...
export class ProfessionnelsDeSanteComponent {
  ...
  get filteredDoctors() {
    ...
    const hasAny = f.symptoms.length || f.approach.length || f.sex.length || f.recommended.length;
  ...
  private matchesDoctor(d: any, f: FiltersPayload): boolean {
    ...
    if (f.recommended.length) {
      ...
      const okPro =
        !f.recommended.includes('Pro') || STRONG_DIPLOMAS.some(sd => lc(d.diplome).includes(sd));
